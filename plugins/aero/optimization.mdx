...
<AccordionGroup>
  ...
  <Accordion title="Defer JavaScript">
    ...
    <Warning>
      Deferring JavaScript can break scripts that need to run during page load — for example, certain analytics tags, chat widgets, or custom theme scripts. Always test JS deferral on a [staging site](/cloud-panel/staging) before enabling it in production. Use the JS exclusion list to carve out any scripts that must load synchronously.
    </Warning>
  </Accordion>
...
<Warning>
  Post revisions are permanently deleted and cannot be recovered from within WordPress after cleanup. Before running a database cleanup that includes revisions, make sure you have a recent site backup on hand. You can create a backup through the [WP Stratos dashboard](/cloud-panel/backups) before proceeding.
</Warning>
