...
The Edge Cache screen (**Aero → Edge Cache**) controls WP Stratos's global edge network, the layer that sits in front of Batcache and serves cached pages from a server physically close to each visitor. This is separate infrastructure from Batcache, which is why it gets its own screen and its own purge step in the [sequential flush order](/plugins/aero/purge-schedule).
