...
<AccordionGroup>
  ...
  <Accordion title="Flush Aero caches after optimization: Safe, recommended">
    Runs the sequential purge (object cache, then Batcache, then Edge) once a bulk optimization run finishes, so pages start serving the new formats immediately instead of showing cached markup that still points at the old files.
  </Accordion>
</AccordionGroup>
