...
keywords: ["fix wordpress email not sending", "smtp authentication error wordpress", "spf dkim setup wordpress email", "email going to spam wordpress"]
---
...
<Steps>
  <Step title="Open WP Postman Email Logs">
    In your WordPress admin, navigate to **Settings → WP Postman → Mail Log**. For a full walkthrough of the logs interface, see the [Email Logs documentation](/plugins/wp-postman/email-logs).
  </Step>
...
<Accordion title="Authentication error">
  An authentication error means the SMTP server accepted the connection but rejected your login credentials. Double-check that the **SMTP username and password** in **Settings → WP Postman → General** exactly match those provided by your email service.
...
<Accordion title='"Could not connect to SMTP" error'>
  ...
  If the error persists on port 587 or 465, open a support request through the Cloud Panel (see the [FAQ](/troubleshooting/faq) for what to include) to confirm there are no outbound SMTP restrictions on your specific plan.
</Accordion>
...
<Steps>
  <Step title="Open the Test Email tab">
    Go to **Settings → WP Postman → Test Email** in your WordPress admin.
  </Step>
  <Step title="Send a test email">
    Fill in the To, Subject, and Message fields, then click **Send Test Email**.
  </Step>
  <Step title="Check the result">
    If the test succeeds, check the recipient inbox (and spam folder). If it fails, a new entry will appear in **Mail Log** with the error, use that to guide your next troubleshooting step.
  </Step>
</Steps>
